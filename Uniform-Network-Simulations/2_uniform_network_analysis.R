...
# Routing analysis
df <- data.frame(Graph = c("G*", "G"), "Routes" = c(99990000, NA), `Avg Hops` = c(4.10, NA), `Max Hops` = c(56, NA),
                 `Avg Propagation` = c(0.65, NA), `Max Propagation` = c(2.76, NA))
...
route <- fread("../C++/greedy_routing_results_1.txt", header = F)
colnames(route) <- c("x", "y")
p1 <- ggplot(nw, aes(x = X, y = Y)) + 
  ggtitle("Route with Maximum Hops") +
  geom_point(color = "grey50", alpha = 0.5) +
  geom_point(data = route, aes(x = x, y = y), color = "red", size = 5) +
  ...
  theme(panel.border = element_rect(colour = "black", fill=NA, size = 1),
        ...
        axis.title = element_text(size = 18),
        plot.title = element_text(size = 20, hjust = 0.5)) + 
  geom_rect(aes(xmin=0.30, xmax=0.4, ymin=0.38, ymax=0.53), fill = NA, color = "black", size = 1)
